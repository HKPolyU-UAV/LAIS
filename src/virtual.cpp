...
//    }
    ...
    if (state == Detection_Success)
    {
      ...
      cv::putText(frame, "State: Detection_success " , cv::Point(20,40), cv::FONT_HERSHEY_SIMPLEX, 0.75, cv::Scalar(255, 0, 0), 2);
      getRandomColors(colors, bboxes.size());
      coretracker->init(bboxes[0],frame);
    ...
    {
      cv::putText(frame, "State: Detection_Failure " , cv::Point(20,40), cv::FONT_HERSHEY_SIMPLEX, 0.75, cv::Scalar(0, 0, 255), 2);
    ...
    if (state == Tracking_Success)
    {
      ...
      tracking_rect = coretracker->update(frame);
      float fps= cv::getTickFrequency() / ((double)cv::getTickCount() - timer);
      cv::rectangle(frame, tracking_rect, cv::Scalar(0,255,0) , 2, 1);
